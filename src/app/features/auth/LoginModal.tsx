// src/app/features/auth/LoginModal.tsx
"use client";
import { useEffect, useSyncExternalStore, useState, type FormEvent } from "react";
import { authModal } from "@/app/shared/store/authModalStore";
import { useAuthStore } from "@/app/shared/store/useAuthstore";
import { FetchService } from "@/app/shared/lib/api/fetch.service";
import { X } from "lucide-react";
import { useTranslations } from "next-intl";

export default function LoginModal() {
  const t = useTranslations();
  const { fetchUser } = useAuthStore();

  // subscribe with SSR-safe getServerSnapshot
  const isOpen = useSyncExternalStore(
    authModal.subscribe.bind(authModal),
    () => authModal.isOpen,
    () => false
  );

  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    if (!isOpen) {
      setUsername("");
      setPassword("");
      setError("");
    }
  }, [isOpen]);

  const close = () => {
    authModal.close();
    authModal.emitLoginCancelled(); // lets queued requests fail cleanly
  };

  const handleGoogleSignIn = () => {
    if (typeof window === "undefined") return;

    // current page (includes locale, path, and query)
    const redirectUrl = window.location.href;

    window.location.href = `https://new-api.openinfo.uz/api/v2/investors/google/login/?result_redirect=${encodeURIComponent(
      redirectUrl
    )}`;
  };

  const submit = async (e: FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setError("");
    try {
      await FetchService.login(username, password);
      await fetchUser();
      authModal.close();
      authModal.emitLoginSuccess();
    } catch {
      setError(t("Login.invalid_credentials"));
    } finally {
      setIsLoading(false);
    }
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 relative border border-default">
        <button onClick={close} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
          <X className="w-5 h-5" />
        </button>

        <h3 className="text-lg font-semibold mb-2">{t("Login.title")}</h3>
        <p className="text-sm text-gray-500 mb-4">{t("Login.subtitle")}</p>

        <form className="space-y-6" onSubmit={submit}>
          <div>
            <label className="block mb-2 text-sm font-semibold text-gray-700">{t("Login.username")}</label>
            <input
              className="w-full px-4 py-3 border rounded-xl"
              value={username}
              onChange={e => setUsername(e.target.value)}
            />
          </div>
          <div>
            <label className="block mb-2 text-sm font-semibold text-gray-700">{t("Login.password")}</label>
            <input
              className="w-full px-4 py-3 border rounded-xl"
              type="password"
              value={password}
              onChange={e => setPassword(e.target.value)}
            />
          </div>

          {error && <div className="text-red-500 text-sm text-center">{error}</div>}

          <div className="flex justify-center gap-4">
            <button type="submit" disabled={isLoading} className="h-12 w-full bg-blue-600 text-white rounded-xl">
              {isLoading ? t("Login.loading") : t("Login.login")}
            </button>
          </div>

          <div>
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={isLoading}
              className="h-12 w-full flex items-center justify-center gap-3 border rounded-xl"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              <span className="text-sm font-medium text-gray-700">{t("Login.login_with_google")}</span>
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}
