...
export const BrokersAutoComplete: React.FC<Props> = ({
  value,
  onChange,
  placeholder = "Наименование услуги",
  className = "w-full",
}) => {
  const [searchTerm, setSearchTerm] = useState(value?.full_name || "");
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      setIsLoading(true);
      try {
        const data = await FetchService.fetch<ResultItem[]>(`/api/v2/brokers/autofill/?name=${searchTerm}`);
        setResults(data);
        setShowResults(true);
      } catch (error) {
          ...
          {results.length > 0 ? (
            results.map(result => (
              ...
              >
                ...
                <span className="whitespace-nowrap">{result.full_name}</span>
              </div>
            ))
          ) : (
...
export default BrokersAutoComplete;
